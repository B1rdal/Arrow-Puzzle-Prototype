...
public class PathArrowAnimation : MonoBehaviour
{
    ...
    private void BuildVisibleWindow(float tailDistance, float headDistance)
    {
        ...
        {
            BuildSegmentMappedFullLengthWindow(travelDistance);
            return;
        }
    ...
    private void BuildSegmentMappedFullLengthWindow(float distanceOffset)
    {
        // Each original segment advances by the same distance along the escape path.
        // If a segment crosses a turn, we insert that real corner so Unity never draws a fake diagonal.
        for (int i = 0; i < sourceDistances.Count - 1; i++)
        {
            float startDistance = sourceDistances[i] + distanceOffset;
            float endDistance = sourceDistances[i + 1] + distanceOffset;
            AddPathInterval(startDistance, endDistance);
        }

        RemoveStraightMiddlePoints();
    }

    private void AddPathInterval(float startDistance, float endDistance)
    {
        AddVisiblePoint(SamplePosition(startDistance));
        ...
        for (int i = 1; i < sourceDistances.Count; i++)
        {
            float cornerDistance = sourceDistances[i];

            if (cornerDistance <= minDistance + MinPointDistance)
            {
                continue;
            }

            if (cornerDistance >= maxDistance - MinPointDistance)
            {
                break;
            }
        ...
        AddVisiblePoint(SamplePosition(endDistance));
    }
