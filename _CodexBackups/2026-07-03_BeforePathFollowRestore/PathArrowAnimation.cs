...
public class PathArrowAnimation : MonoBehaviour
{
    ...
    private void BuildVisibleWindow(float tailDistance, float headDistance)
    {
        ...
        {
            BuildRigidFullLengthWindow(tailDistance);
            return;
        }
    ...
    private void BuildRigidFullLengthWindow(float travelOffset)
    {
        // Full-length exits keep the original points and move the whole arrow as one shape.
        // This avoids LineRenderer corner rebuilding, which can make turns twitch or break apart.
        Vector3 offset = exitDirection * travelOffset;

        for (int i = 0; i < sourcePoints.Count; i++)
        {
            AddVisiblePoint(sourcePoints[i] + offset);
        }
    }
